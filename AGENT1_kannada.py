"""
AGENT1 — Agentic AI Demo with Kannada Explanations
...
# ─── KANNADA CONCEPTS ──────────────────────────────────────────────────────────
TE = {
    "agent":    ("AI Agent ಎಂದರೇನು?",
                 "AI Agent ಸ್ವತಃ ನಿರ್ಧಾರಗಳನ್ನು ತೆಗೆದುಕೊಂಡು Tools ಬಳಸಿ ಗುರಿಯನ್ನು ತಲುಪುವ system. "
                 "Chatbot ಕೇವಲ Q&A ಮಾಡುತ್ತದೆ — Agent plan, act, observe ಮಾಡುತ್ತದೆ!"),
    "react":    ("ReAct ಎಂದರೇನು?",
                 "ReAct = Reasoning + Acting. Agent: Thought → Action → Observation loop ನಲ್ಲಿ "
                 "ಕೆಲಸ ಮಾಡುತ್ತದೆ. ಉತ್ತರ ಸಿಗುವವರೆಗೆ ಈ cycle repeat ಆಗುತ್ತದೆ."),
    "tools":    ("Tools ಎಂದರೇನು?",
                 "Tools = Agent ಬಳಸುವ special capabilities. Calculator, Converter, Analyzer — "
                 "Agent ಸಮಸ್ಯೆಗೆ ತಕ್ಕ tool ಅನ್ನು automatically select ಮಾಡುತ್ತದೆ."),
    "planning": ("Planning ಎಂದರೇನು?",
                 "Agent ದೊಡ್ಡ task ಅನ್ನು ಸಣ್ಣ steps ಆಗಿ break ಮಾಡುತ್ತದೆ. ಯಾವ tool ಅನ್ನು ಯಾವಾಗ "
                 "ಬಳಸಬೇಕು ಎಂದು decide ಮಾಡುತ್ತದೆ — Human problem-solving ಹಾಗೆ!"),
    "obs":      ("Observation ಎಂದರೇನು?",
                 "Tool result ಅನ್ನು Agent observe ಮಾಡುತ್ತದೆ. Correct ಆದರೆ Final Answer. "
                 "ಇಲ್ಲದಿದ್ದರೆ ಮತ್ತೆ different approach try ಮಾಡುತ್ತದೆ — Self-correction!"),
    "agentic":  ("Agentic AI ಎಂದರೇನು?",
                 "2026 ರಲ್ಲಿ ಅತಿ ದೊಡ್ಡ trend! Complex multi-step tasks autonomously complete "
                 "ಮಾಡುವ AI systems. Chatbots ಗಿಂತ ಬಹಳ powerful!"),
    "llm":      ("LLM ಎಂದರೇನು?",
                 "Large Language Model — Agent ನ brain. Tool ಅನ್ನು ಯಾವಾಗ ಬಳಸಬೇಕು, "
                 "ಯಾವ input ಕೊಡಬೇಕು, final answer ಏನು ಎಂಬುದನ್ನು ಇದು decide ಮಾಡುತ್ತದೆ."),
    "memory":   ("Agent Memory ಎಂದರೇನು?",
                 "Agent conversation history remember ಮಾಡುತ್ತದೆ. Previous steps, tools "
                 "used, observations — ಎಲ್ಲವೂ memory ನಲ್ಲಿ ಇರುತ್ತವೆ. Multi-turn possible!"),
}

def tip(key):
    """Return Kannada hover tooltip HTML"""
    t, b = TE[key]
    return (f'<span class="tc"><span class="badge">ಕಂ</span>'
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Kannada Explanations ಜೊತೆ</span>",
        unsafe_allow_html=True)

# ─── HEADER ───────────────────────────────────────────────────────────────────
st.markdown('<div class="app-title">🤖 AGENT1</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="app-sub">Agentic AI Demo — Kannada Explanations ಜೊತೆ | '
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown("## 🧠 Agentic AI Concepts — Kannada లో ಕಲಿಯೋಣ!")
...
Example:
  Step 1: Calculator ಜೊತೆ 'sqrt 144' try చేసింది → Error
    ...
    for icon, name, fn, desc_en, desc_te in TOOLS_META:
        ...
        with c2:
            st.markdown(
                ...
                f'<div class="tool-te">ಕಂ: {desc_te}</div>'
                f'</div>',
                unsafe_allow_html=True)
        st.markdown("")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown("## 📊 Chatbot vs AI Agent — ವ್ಯತ್ಯಾಸ ಏನು?")
    st.markdown("")
    ...
    <div class="concept-card">
        <h3>🔑 Key Insight — Kannada లో Summary</h3>
        <p class="telugu">
        ...
        📈 <b>2026 Industry Trend:</b> Chatbots నుండి Agents కి rapid shift జరుగుಜೊತೆంది.<br>
